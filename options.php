...
if ($RIGHT >= "R") :
    $arAllOptions = Array(
        array("API_KEY", "API KEY", array("text", 64), ""),
        array("IBLOCK_ID", "Инфоблок", array("select")),
        array("AUTH_API", "Адрес API для авторизации", array("text", 64)),
        array("EXPORT_API", "Адрес API для экспорта", array("text", 64)),
    );
    ...
    if ($REQUEST_METHOD == "POST" && strlen($Update . $Apply . $RestoreDefaults) > 0 && $RIGHT == "W" && check_bitrix_sessid()) {
        ...
        if (strlen($RestoreDefaults) > 0) {
            COption::RemoveOption("promotions", "API_KEY");
        } else {
                ...
                <td width="60%">
                    ...
                    <? elseif ($type[0] == "text"):?>
                        <input type="text" size="<? echo $type[1] ?>" maxlength="255"
                               ...
                               id="<? echo htmlspecialcharsbx($arOption[0]) ?>"><? if ($arOption[0] == "slow_sql_time") echo GetMessage("PERFMON_OPTIONS_SLOW_SQL_TIME_SEC") ?>
                    ...
                    <? elseif ($type[0] == "select"):?>
                        <select name="<? echo htmlspecialcharsbx($arOption[0]) ?>"
                            ...
                            <? while ($iblock = $iblocks->fetch()): ?>
                                <option name="<? echo $iblock["CODE"] ?>"
                                        <?if($iblock["ID"] == $val):?>
                                        selected
                                        <?endif?>
                                        value="<? echo $iblock["ID"] ?>"> <? echo $iblock["NAME"] ?> </option>
                            <? endwhile; ?>
                        </select>
                    <? endif ?>
                </td>
